...
SlashCmdList['PIZZAWORLDBUFFS'] = function (args, editbox)
  ...
  if command == 'log' then
    ...
    PWB_config.log = number == 1
    local message
    if PWB_config.log then
      message = T['Timer report logging enabled']
    else
      message = T['Timer report logging disabled']
    end
    PWB:Print(message)
    return
  end
  ...
  if command == 'time' then
    ...
    if not h or not m then
      PWB:Print("Server time not available. Please type .server info first to get server time.")
    else
      local timeStr = string.format("%02d:%02d", h, m)
      PWB:Print("Server time: " .. timeStr)
    end
    return
  end

  if command == 'share' or command == 'publish' then
    PWB.core.publishAll()
    PWB:Print(T['Sharing timers with other addon users'])
    return
  end
  ...
  PWB:PrintClean(PWB.Colors.primary .. '   /wb|r log ' .. (PWB_config.log and 1 or 0) .. PWB.Colors.grey .. ' - ' .. T['Toggle timer report logging to chat'])
  PWB:PrintClean(PWB.Colors.primary .. '   /wb|r share ' .. PWB.Colors.grey .. '- ' .. T['Manually share timers with other addon users'])
  PWB:PrintClean(PWB.Colors.primary .. '   /wb|r forceao ' .. PWB.Colors.grey .. '- ' .. T['Force create Alliance Onyxia timer for testing'])
end
